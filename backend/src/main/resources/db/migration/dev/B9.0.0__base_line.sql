...
(
    discount_details_id     int unsigned auto_increment comment 'Mã chi tiết giảm giá'
        primary key,
    discount_type_id        smallint unsigned                    null comment 'Mã loại giảm giá',
...
(
    ...
    discount_details_id   int unsigned                           null comment 'Mã chi tiết giảm giá',
    min_prod_nums         tinyint unsigned                       not null comment 'Số lượng sản phẩm tối thiểu',
    valid_from            datetime                               not null comment 'Ngày bắt đầu hiệu lực',
    valid_until           datetime                               not null comment 'Ngày kết thúc hiệu lực',
    current_uses          int unsigned default '0'               null comment 'Số lần đã sử dụng',
    max_uses              int unsigned                           null comment 'Số lần sử dụng tối đa',
    max_uses_per_customer smallint unsigned                      null comment 'Số lần sử dụng tối đa cho mỗi khách hàng',
    is_active             tinyint(1)   default 1                 null comment 'Trạng thái (1: Hoạt động, 0: Không hoạt động)',
    created_at            datetime     default CURRENT_TIMESTAMP null comment 'Ngày tạo',
    updated_at            datetime     default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP,
...
(
    product_price_discount_id int unsigned auto_increment comment 'Mã giảm giá theo giá sản phẩm'
        primary key,
    product_price_id          int unsigned                         null comment 'Mã giá sản phẩm',
    discount_details_id       int unsigned                         null comment 'Mã chi tiết giảm giá',
    min_required_prod         smallint unsigned                    not null comment 'Số lượng sản phẩm tối thiểu',
    valid_from                datetime                             not null comment 'Ngày bắt đầu hiệu lực',
    valid_until               datetime                             not null comment 'Ngày kết thúc hiệu lực',
...
(
    topping_id   int unsigned auto_increment comment 'Mã topping'
        primary key,
    name         varchar(50)                          not null comment 'Tên topping',
    description  text                                 null comment 'Mô tả',
    price        decimal(10, 3)                       not null comment 'Giá',
...
(
    order_topping_id int auto_increment comment 'Mã topping chi tiết đơn hàng'
        primary key,
    order_product_id int unsigned                       null comment 'Mã sản phẩm đơn hàng',
    topping_id       int unsigned                       null comment 'Mã topping',
    quantity         smallint unsigned                  not null comment 'Số lượng',
    created_at       datetime default CURRENT_TIMESTAMP null,
    updated_at       datetime default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP,
