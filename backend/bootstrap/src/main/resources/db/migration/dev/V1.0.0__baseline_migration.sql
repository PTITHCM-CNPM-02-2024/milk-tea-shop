...
--  check (`apply_type` in (_utf8mb4\'BEST\',_utf8mb4\'COMBINE\'))
);
...
(
    emp_prof_id       int auto_increment comment 'Mã hồ sơ nhân viên'
        primary key,
    employee_id       int                                null comment 'Mã nhân viên',
    salary            decimal(10, 2)                     not null comment 'Lương',
    hire_date         date                               not null comment 'Ngày bắt đầu làm việc',
    emergency_contact varchar(100)                       null comment 'Liên hệ khẩn cấp',
    bank_account      varchar(50)                        null comment 'Số tài khoản ngân hàng',
    tax_code          varchar(20)                        null comment 'Mã số thuế',
    created_at        datetime default CURRENT_TIMESTAMP null,
    updated_at        datetime default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP,
    constraint employee_id
        unique (employee_id),
    constraint EmployeeProfile_ibfk_1
...
(
    order_item_id    int auto_increment comment 'Mã chi tiết đơn hàng'
        primary key,
...
create index product_price_id
    on OrderDetail (product_price_id);
...
(
    table_status_id int auto_increment comment 'Mã trạng thái bàn'
        primary key,
    table_id        int                                        null comment 'Mã bàn',
    status          enum ('AVAILABLE', 'OCCUPIED', 'RESERVED') not null comment 'Trạng thái bàn (AVAILABLE, OCCUPIED, RESERVED)',
    updated_at      timestamp default CURRENT_TIMESTAMP        null on update CURRENT_TIMESTAMP comment 'Thời gian cập nhật',
    created_at      datetime  default CURRENT_TIMESTAMP        null,
    constraint TableStatus_ibfk_1
        foreign key (table_id) references ServiceTable (table_id)
);

create index table_id
    on TableStatus (table_id);
...
create table if not exists OrderDetailTopping
(
    order_detail_topping_id int auto_increment comment 'Mã topping chi tiết đơn hàng'
        primary key,
    order_item_id           int                                null comment 'Mã chi tiết đơn hàng',
    topping_id              int                                null comment 'Mã topping',
    quantity                int                                not null comment 'Số lượng',
    created_at              datetime default CURRENT_TIMESTAMP null,
    updated_at              datetime default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP,
    constraint OrderDetailTopping_ibfk_1
        foreign key (order_item_id) references OrderDetail (order_item_id),
    constraint OrderDetailTopping_ibfk_2
        foreign key (topping_id) references Topping (topping_id)
);

create index order_item_id
    on OrderDetailTopping (order_item_id);

create index topping_id
    on OrderDetailTopping (topping_id);
