...
BEGIN
    ...
    DECLARE @test_file NVARCHAR(500);
    DECLARE @database_name NVARCHAR(100) = N'tempdb';
    
    -- Thử tạo một file test nhỏ để kiểm tra directory
    SET @test_file = @backup_path + 'test_access.tmp';
    
    BEGIN TRY
        -- Thử backup nhỏ để test directory access
        BACKUP DATABASE @database_name 
        TO DISK = @test_file
        WITH FORMAT, INIT, COPY_ONLY, SKIP, 
             NAME = 'Directory Test',
             DESCRIPTION = 'Test backup directory access';
        
        -- Xóa file test
        DECLARE @del_cmd NVARCHAR(1000) = 'rm -f "' + @test_file + '"';
        -- EXEC xp_cmdshell @del_cmd, NO_OUTPUT;
        
        PRINT N'✅ Backup directory accessible: ' + @backup_path;
        RETURN 1; -- Success
        
    END TRY
    BEGIN CATCH
        DECLARE @error_msg NVARCHAR(500) = ERROR_MESSAGE();
        PRINT N'❌ Backup directory không accessible: ' + @error_msg;
        
        -- Nếu lỗi directory, thử fallback
        IF @error_msg LIKE '%cannot open backup device%' OR @error_msg LIKE '%path%'
        BEGIN
            PRINT N'💡 Sẽ sử dụng fallback directory: /var/opt/mssql/data/';
        END
...
BEGIN
    ...
    SET @filename = @database_name + '_FULL_' + @date_stamp + '.bak';
    ...
    BEGIN TRY
        ...
        TO DISK = @full_path
        WITH FORMAT, INIT, 
             NAME = @database_name + ' Full Backup',
             DESCRIPTION = 'Full backup của ' + @database_name + ' lúc ' + CONVERT(NVARCHAR(19), GETDATE(), 120),
             COMPRESSION,
             CHECKSUM,
             STATS = 10;
...
BEGIN
    ...
    -- Tạo timestamp cho filename
    SET @date_stamp = FORMAT(GETDATE(), 'yyyyMMdd_HH') + '00';
    SET @filename = @database_name + '_DIFF_' + @date_stamp + '.bak';
    SET @full_path = @backup_path + @filename;
    ...
    BEGIN TRY
        ...
        WITH DIFFERENTIAL, FORMAT, INIT,
             NAME = @database_name + ' Differential Backup',
             DESCRIPTION = 'Differential backup của ' + @database_name + ' lúc ' + CONVERT(NVARCHAR(19), GETDATE(), 120),
             COMPRESSION,
             CHECKSUM,
             STATS = 10;
...
BEGIN
    ...
    BEGIN TRY
        ...
        PRINT N'✅ Đã cleanup ' + CAST(@deleted_count AS NVARCHAR(10)) + ' backup files cũ';
