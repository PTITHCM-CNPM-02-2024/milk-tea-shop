...
do
        echo $i 
        mysql -h $i -uroot -p$MYSQL_ROOT_PASSWORD -P3306 -e "CREATE USER IF NOT EXISTS '$MYSQL_PROXY_USER'@'$ipaddr' IDENTIFIED BY '$MYSQL_PROXY_PASSWORD'"
        mysql -h $i -uroot -p$MYSQL_ROOT_PASSWORD -P3306 -e "GRANT ALL ON *.* TO '$MYSQL_PROXY_USER'@'$ipaddr'"
        echo "done"
