...
export const useStoreInfoStore = defineStore('storeInfo', () => {
  ...
  async function fetchStoreInfo() {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Đã xảy ra lỗi khi tải thông tin cửa hàng'
      throw err
    } finally {
      loading.value = false
    }
  ...
  async function updateStoreInfo(data) {
    ...
    try {
      ...
      error.value = err.response?.data?.message || 'Đã xảy ra lỗi khi cập nhật thông tin cửa hàng'
      throw err
    } finally {
      loading.value = false
    }
