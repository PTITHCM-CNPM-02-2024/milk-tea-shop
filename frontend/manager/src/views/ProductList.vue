  ...
  <div>
      ...
      <v-window-item value="products">
        <v-card>
          <v-card-title class="d-flex justify-space-between align-center py-3 px-4">
            ...
            <v-btn color="primary" size="small" variant="text" prepend-icon="mdi-package-variant-plus"
              @click="openAddDialog">
              Thêm mới
            </v-btn>
          </v-card-title>
          ...
          <v-card-text class="pa-2">
            ...
            <v-row class="px-2 py-2">
              <v-col cols="12" md="4">
                <v-text-field v-model="searchQuery" label="Tìm kiếm" density="compact" variant="outlined" hide-details
                  prepend-inner-icon="mdi-magnify" clearable @update:model-value="debounceSearch"></v-text-field>
              </v-col>

              <v-col cols="12" md="3">
                <v-select v-model="selectedCategory" :items="categoryOptions" label="Danh mục" density="compact"
                  variant="outlined" hide-details clearable></v-select>
              </v-col>

              <v-col cols="12" md="3">
                <v-select v-model="selectedStatus" :items="statusOptions" label="Trạng thái" density="compact"
                  variant="outlined" hide-details clearable></v-select>
              </v-col>

              <v-col cols="12" md="2">
                <v-btn color="primary" variant="outlined" block @click="resetFilters" :disabled="!hasFilters"
                  density="compact">
                  Đặt lại
                </v-btn>
              </v-col>
            ...
            <v-row class="px-2">
              <v-col v-for="product in paginatedProducts" :key="product.id" cols="12" sm="6" md="6" lg="4">
                <v-card class="h-100">
                  <v-img :src="product.image_url || '/images/placeholder.png'" cover height="180"
                  ...
                  <v-card-title class="d-flex justify-space-between align-center py-2">
                    ...
                    <v-chip v-if="product.catId" size="small" label color="primary" variant="outlined" class="ml-2">
                  ...
                  <v-card-text class="pt-1 pb-2">
                    ...
                    <div v-if="product.prices && product.prices.length > 0" class="d-flex flex-wrap gap-1 mt-2">
                      <v-chip v-for="price in product.prices.slice(0, 3)" :key="price.id" size="x-small" color="success"
                        variant="flat" class="font-weight-medium">
                        {{ price.size }}: {{ formatCurrency(price.price) }}
                      </v-chip>
                      <v-chip v-if="product.prices.length > 3" size="x-small" color="grey" variant="flat"
                  ...
                  <v-card-actions>
                    ...
                    <v-btn icon="mdi-pencil" size="small" color="primary" variant="text"
                      @click="openEditDialog(product)"></v-btn>
                    <v-btn icon="mdi-delete" size="small" color="error" variant="text"
                      @click="openDeleteDialog(product)"></v-btn>
                  </v-card-actions>
                </v-card>
              </v-col>

              <!-- Hiển thị khi không có dữ liệu -->
              <v-col v-if="!productStore.loading && filteredProducts.length === 0" cols="12" class="text-center py-6">
                ...
                <v-btn color="primary" size="small" class="mt-2" prepend-icon="mdi-package-variant-plus"
                  @click="openAddDialog">
                  Thêm sản phẩm
                </v-btn>
              </v-col>
            ...
            <v-row v-if="filteredProducts.length > 0" class="py-4">
              <v-col cols="12" class="d-flex justify-center">
                <v-pagination v-model="productPage" :length="totalProductPages" :total-visible="5"
                  @update:model-value="handleProductPageChange" rounded></v-pagination>
              </v-col>
            </v-row>
          </v-card-text>
        </v-card>
      ...
      <v-window-item value="categories">
        <v-card class="mb-4">
          <v-card-title class="d-flex justify-space-between align-center py-3 px-4">
            ...
            <v-btn color="primary" size="small" variant="text" prepend-icon="mdi-shape-plus"
          ...
          <v-card-text class="pa-2">
            <v-list lines="two" class="py-0">
              <v-list-item v-for="category in productStore.categories" :key="category.id" :title="category.name"
                :subtitle="category.description || 'Không có mô tả'" :value="category.id" rounded="lg" class="mb-1"
                ...
                <template v-slot:append>
                  <div class="d-flex gap-1">
                    <v-btn icon="mdi-pencil" size="x-small" color="primary" variant="text"
                      @click.stop="openEditCategoryDialog(category)"></v-btn>
                    <v-btn icon="mdi-delete" size="x-small" color="error" variant="text"
                      @click.stop="openDeleteCategoryDialog(category)"></v-btn>
                  </div>
                </template>
              </v-list-item>
              ...
              <div v-if="productStore.categories.length === 0 && !productStore.loading" class="text-center py-6">
                ...
                <v-btn color="primary" size="small" class="mt-2" prepend-icon="mdi-shape-plus"
                  @click="openAddCategoryDialog">
                  Thêm danh mục
                </v-btn>
              </div>
            ...
            <v-row v-if="productStore.categories.length > 0" class="py-4">
              <v-col cols="12" class="d-flex justify-center">
                <v-pagination v-model="categoryPage" :length="Math.ceil(categoryTotal / categoryPageSize)"
                  :total-visible="5" @update:model-value="handleCategoryPageChange" rounded></v-pagination>
              </v-col>
            </v-row>
          </v-card-text>
        </v-card>
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form ref="addForm" @submit.prevent="saveProduct">
            <!-- Hiển thị lỗi dialog -->
            <v-alert v-if="productDialogError" type="error" variant="tonal" closable class="mb-4"
            ...
            <v-select v-model="editedProduct.categoryId" :items="categoryOptions" label="Danh mục" variant="outlined"
              item-title="title" item-value="value" class="mb-3" clearable></v-select>

            <div class="d-flex gap-3 mb-3">
              <v-switch v-model="editedProduct.signature" label="Sản phẩm đặc trưng" color="primary"
                hide-details></v-switch>

              <v-switch v-model="editedProduct.available" label="Sản phẩm có sẵn" color="success"
                hide-details></v-switch>

              <v-switch v-model="editedProduct.isTopping" label="Là topping" color="info" hide-details
                @update:model-value="handleToppingChange"></v-switch>
            </div>
            ...
            <div class="mb-3">
              ...
              <div class="d-flex flex-column flex-md-row align-center gap-4">
                <v-img :src="editedProduct.imageUrl || '/images/placeholder.png'" width="120" height="120" cover
                  class="bg-grey-lighten-2 rounded elevation-1"></v-img>

                <div class="flex-grow-1">
                  <v-card variant="outlined" class="pa-3">
                    <v-file-input v-model="productImage" accept="image/*" label="Tải lên hình ảnh" variant="outlined"
                      density="compact" prepend-icon="mdi-camera" :show-size="true"
                      @update:model-value="handleImageChange" hint="Chọn ảnh có kích thước tối ưu 500x500px"
                    ...
                    <div v-if="editedProduct.imageUrl" class="mt-2 d-flex justify-end">
                      <v-btn size="small" color="error" variant="text" density="compact" prepend-icon="mdi-delete"
                        @click="removeImage">
                        Xóa ảnh
                      </v-btn>
                    </div>
            ...
            <v-card v-if="!editDialog" variant="outlined" class="mt-3 mb-3">
              <v-card-title class="d-flex justify-space-between align-center py-3">
                ...
                <v-btn size="small" color="primary" variant="text" prepend-icon="mdi-plus" @click="addProductPrice">
                  Thêm giá
                </v-btn>
              </v-card-title>
                ...
                <v-table density="compact">
                  ...
                  <tbody>
                    <tr v-for="(price, index) in editedProduct.prices" :key="index">
                      <td>
                        <v-select v-model="price.sizeId" :items="productStore.productSizes" item-title="name"
                          item-value="id" variant="plain" density="compact" hide-details class="ma-1"></v-select>
                      </td>
                      <td>
                      ...
                      <td class="text-center">
                        <v-btn icon="mdi-delete" size="x-small" color="error" variant="text"
                          @click="removeProductPrice(index)"></v-btn>
                      </td>
                    </tr>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="saveProduct" :loading="productStore.loading">
            Lưu
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="editDialog" width="700" persistent>
      <v-card>
                ...
                <v-table density="compact">
                  ...
                  <tbody>
                    <tr v-for="(price, index) in editedProduct.prices" :key="index">
                      ...
                      <td class="text-center">
                        <v-btn icon="mdi-delete" size="x-small" color="error" variant="text"
                          @click="confirmDeletePrice(price)"></v-btn>
                      </td>
                    </tr>
                    <tr v-if="editedProduct.prices.length === 0">
                      <td colspan="3" class="text-center py-3 text-medium-emphasis">
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="updateProduct" :loading="productStore.loading">
            Cập nhật
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="deleteDialog" width="400" persistent>
      <v-card>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="error" @click="deleteSelectedProduct" :loading="productStore.loading">
            Xóa
          </v-btn>
        </v-card-actions>
      </v-card>
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form ref="addCategoryForm" @submit.prevent="saveCategory">
            <!-- Hiển thị lỗi dialog -->
            <v-alert v-if="categoryDialogError" type="error" variant="tonal" closable class="mb-4"
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="saveCategory" :loading="productStore.loading">
            Lưu
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="editCategoryDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="updateCategory" :loading="productStore.loading">
            Cập nhật
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="deleteCategoryDialog" width="400" persistent>
      <v-card>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="error" @click="deleteSelectedCategory" :loading="productStore.loading">
            Xóa
          </v-btn>
        </v-card-actions>
      </v-card>
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          <v-form ref="addPriceForm" @submit.prevent="saveProductPrice">
            <!-- Hiển thị lỗi dialog -->
            <v-alert v-if="priceDialogError" type="error" variant="tonal" closable class="mb-4"
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" @click="saveProductPrice" :loading="productStore.loading">
            Thêm
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="deletePriceDialog" width="400" persistent>
      <v-card>
        ...
        <v-card-text class="pa-4 pt-5">
          <p class="text-justify text-medium-emphasis mt-2">Bạn có chắc chắn muốn xóa giá cho kích cỡ <strong>{{ selectedPrice ? getSizeName(selectedPrice.sizeId) :
              ''
              }}</strong>?
          </p>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="error" @click="deletePrice" :loading="productStore.loading">
            Xóa
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="toppingConfirmationDialog" width="450" persistent>
      <v-card>
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="warning" @click="confirmToppingChange">
            Xác nhận
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-snackbar v-model="snackbar.show" :color="snackbar.color" :timeout="3000">
      ...
      <template v-slot:actions>
        <v-btn color="white" icon="mdi-close" variant="text" @click="snackbar.show = false"></v-btn>
      </template>
    </v-snackbar>
  </div>
</template>
