    ...
    <v-card>
      <v-card-text>
        ...
        <div class="d-flex justify-space-between align-center mb-6">
          <div>
            <h1 class="text-h5 font-weight-bold">Quản lý tài khoản</h1>
...
const roleOptions = computed(() => {
  return accountStore.roles.map(role => ({
...
function openAddDialog() {
  editedAccount.value = {
    ...
    isActive: true,
    isLocked: false,
    role: accountStore.roles.find(r => r.name === 'CUSTOMER'),
    password: ''
  }
...
function openEditDialog(item) {
  ...
  editedAccount.value = { 
    ...
    role: item.role ? accountStore.roles.find(r => r.id === item.role.id) : null,
...
async function saveAccount() {
  ...
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể tạo tài khoản mới. Vui lòng thử lại', 'error')
  }
}
...
async function updateAccount() {
  ...
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể cập nhật tài khoản. Vui lòng thử lại', 'error')
  }
}
...
async function deleteSelectedAccount() {
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể xóa tài khoản. Vui lòng thử lại', 'error')
  }
}
...
async function toggleLock(item) {
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể thay đổi trạng thái khóa tài khoản. Vui lòng thử lại', 'error')
  }
}
...
async function toggleActive(item) {
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể thay đổi trạng thái kích hoạt tài khoản. Vui lòng thử lại', 'error')
  }
}
...
onMounted(async () => {
  try {
    ...
    showSnackbar(error.response?.data?.message || 'Không thể tải danh sách vai trò.', 'error')
  }
